...
//        printf("Connessione accettata\n");
        if (readn(connsd,buff,sizeof(char)*MAXLINE)<0){
        ...
        if (carico==1 && pthread_cond_signal(&cond)!=0){
            perror("cond");
            exit(1);
        }
...
//        if (writen(connsd, buff, strlen(buff)+1) <0) {
//            perror("errore in write");
//            exit(1);
//        }
        fprintf(stderr,"byte scritti: %d\n",writen(connsd, buff, strlen(buff)+1));
...
//            perc=perc1;
            send2->fill=perc1;
//            printf("perc=%d\n",perc);
            if (writen(sockfd, send2, sizeof(int)+1) <0) {
                perror("errore in write");
                exit(1);
            }
...
int main(int argc, char **argv)
{
    ...
    if (readn(connsd,cluster,sizeof(struct setup_cluster))<0){
        perror("Errore nella read");
        exit(EXIT_FAILURE);
    }
